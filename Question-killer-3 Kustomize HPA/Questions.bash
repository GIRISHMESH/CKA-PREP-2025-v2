...
Modify the existing configuration to replace the application's current external autoscaling configuration with a Kubernetes-based autoscaling configuration.

Your changes must satisfy the following requirements:

1. The existing horizontal scaling configuration must no longer be used.
...
* Do not create a separate configuration outside the existing application structure.
